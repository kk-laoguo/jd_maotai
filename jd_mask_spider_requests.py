...
import requests
from util import parse_json, get_session,send_wechat,response_status,save_image,open_image,check_login
...
class Jd_Mask_Spider(object):
    def __init__(self):
        ...
        self.is_login = False
        self.time_diff = 0.1
        print("京东时间:%s\n本地时间:%s"%(datetime.fromtimestamp(self.getTime()),datetime.now()))
        try:
            self._load_cookies()
        except Exception:
            pass
